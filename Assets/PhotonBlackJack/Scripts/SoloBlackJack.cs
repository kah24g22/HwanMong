...
public class SoloBlackJack : MonoBehaviour
{
    ...
    private void UpdateItemUI()
    {
        ...
        for (int i = 0; i < m_itemTexts.Length; i++)
        {
            ...
            {
                ItemType itemType = m_myBlackJackPlayer.Inventory[i];
                m_itemTexts[i].text = itemType.ToString();
                // 아이템 아이콘 로드 및 설정
                Sprite itemSprite = Resources.Load<Sprite>($"Sprites/item/{itemType.ToString()}");
                if (m_itemIcons != null && i < m_itemIcons.Length && m_itemIcons[i] != null)
                {
                    m_itemIcons[i].sprite = itemSprite;
                    m_itemIcons[i].gameObject.SetActive(true);
                }
            }
            else
            {
                ...
                if (m_itemIcons != null && i < m_itemIcons.Length && m_itemIcons[i] != null)
                {
                    m_itemIcons[i].sprite = null;
                    m_itemIcons[i].gameObject.SetActive(false);
                }
            }
        }
    }
